...
function SectionHeader({ title, description }: { title: string; description?: string }) {
  return (
    <div className="border-b border-[color:var(--color-ink)]/10 pb-2">
      ...
      {description && (
        <p className="mt-0.5 text-sm text-[color:var(--color-muted)]">{description}</p>
      )}
    </div>
  );
}
  ...
  return (
    <section className="max-w-3xl space-y-10">
      ...
      <div className="space-y-4">
        <SectionHeader
          title="Key details"
          description="Couple names, dates, and the nav logo shown across the site."
        />
        <SettingForm
          settingKey="site_title"
          label="Couple names (site title)"
          inputType="text"
          currentValue={
      ...
      <div className="space-y-4">
        <SectionHeader
          title="Advanced"
          description="Browser tab, SEO meta tags, and page copy."
        />
        <SettingForm
